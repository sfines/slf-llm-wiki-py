...
from src.models.document import RawFile, HashedDocument, SummarizedDocument, IndexedDocument
from src.core.nodes import parser_node, hashing_node
...
class WikiPipeline:
    ...
    async def cache_check_node(self, hashed_doc: HashedDocument) -> Event:
        ...
        if existing_entry and Path(existing_entry.summary_path).exists():
            logger.info(f"Skipping {hashed_doc.source_path.name} (already in index: {Path(existing_entry.summary_path).name})")
    ...
    async def generator_node(self, hashed_doc: HashedDocument) -> Event:
        ...
        if hashed_doc.frontmatter.get('_skip_generation'):
            return Event(output=SummarizedDocument(
        ...
        summary = await self.generation_workflow.process_document(
            document_id=hashed_doc.document_id,
            filename=hashed_doc.source_path.name,
            body=hashed_doc.body
        )
    ...
    async def generate_pages(self, force: bool = False):
        ...
        for raw_file in raw_files:
            ...
            events = runner.run_async(
                new_message=raw_file
            )
            async for _ in events:
                # ADK 2.0 will flow through parser_node -> hashing_node -> cache_check_node -> generator_node -> indexer_node
                pass
