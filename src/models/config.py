...
class LLMConfig(BaseModel):
    backend: str = Field(default="openrouter", description="The LLM backend to use (openrouter, openai, bedrock, ollama, vertex)")
    model_id: str = Field(default="mistralai/mistral-7b-instruct:free", description="The model ID to use for generation")
...
class WikiConfig(BaseModel):
    ...
    content_dir: Path = Field(default=Path("content_new"), description="The input directory containing the raw content folders")
...
def load_config(config_path: Path) -> WikiConfig:
    ...
    if hasattr(module, "config"):
        ...
        raise ValueError(f"Configuration file {config_path} must define a 'config' variable or 'get_config()' function.")
