...
class WikiChatApp:
    ...
    def _build_ui(self):
        ...
        with ui.row().classes("w-full max-w-2xl mx-auto fixed bottom-4 items-center"):
            user_input = ui.input(placeholder="Ask the wiki...").classes("flex-grow").on('keydown.enter', lambda e: send_message())
            ...
            async def send_message():
                ...
                if not results:
                    with chat_messages:
                        ui.markdown("**Wiki:** No relevant context found in the wiki.").classes("bg-gray-100 p-2 rounded-lg")
                    return
