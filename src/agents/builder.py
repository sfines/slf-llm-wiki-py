...
def create_writer_agent(config: LLMConfig, custom_instruction: Optional[str] = None) -> Agent:
    instruction = custom_instruction or """You are an expert technical writer and archivist building an LLM-Wiki.
...
def create_evaluator_agent(config: LLMConfig, custom_instruction: Optional[str] = None) -> Agent:
    instruction = custom_instruction or """You are an expert editor and reviewer.
...
def create_editor_agent(config: LLMConfig, custom_instruction: Optional[str] = None) -> Agent:
    instruction = custom_instruction or """You are an expert technical editor.
...
def create_indexer_agent(config: LLMConfig, custom_instruction: Optional[str] = None) -> Agent:
    instruction = custom_instruction or """You are a knowledge graph taxonomy builder.
Given a list of existing wiki pages and their descriptions, generate a cohesive hierarchical `index.md` file.
Structure it with logical headings and bullet points linking to the pages (e.g., `- [Page Title](page-file-name.md)`)."""
...
def create_repair_agent(config: LLMConfig, custom_instruction: Optional[str] = None) -> Agent:
    instruction = custom_instruction or """You are a markdown repair agent.
You are given a broken markdown file and a linting report showing structural issues (e.g. broken links, unclosed fences, missing headings).
...
def create_chat_agent(config: LLMConfig) -> Agent:
    return Agent(
        name="chat_agent",
        instruction="You are a helpful assistant specialized in answering questions about the wiki content provided in the context.",
        model=get_model(config)
    )
