...
class WikiLogger:
    ...
    def log_summary(self, action: str, status: str, details: Dict[str, Any]):
        ...
        entry = {"run_id": self.run_id, "action": action, "status": status, "details": details}
        with open(self.summary_file, "a", encoding="utf-8") as f:
            f.write(json.dumps(entry) + "\n")

    def log_detail(self, file_id: str, stage: str, status: str, metrics: Optional[Dict[str, Any]] = None):
        """Logs specific file-level processing details (e.g. LLM calls)."""
        entry = {"run_id": self.run_id, "file_id": file_id, "stage": stage, "status": status, "metrics": metrics or {}}
