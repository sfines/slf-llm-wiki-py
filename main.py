...
@click.group()
@click.option(
    "--config", default="config/wiki_config.py", type=click.Path(exists=False), help="Path to Python config file"
)
...
def topics(ctx):
    ...
    index_path = ctx.obj.wiki_dir / "index.json"
    indexer = JsonIndexer(index_path)
    taxonomy_builder = TaxonomyBuilder(ctx.obj, indexer)
...
if __name__ == "__main__":
    if "GOOGLE_APPLICATION_CREDENTIALS" not in os.environ and "VERTEX_PROJECT" not in os.environ:
        click.secho(
            "Warning: Make sure you are authenticated with Vertex AI "
            "(e.g. gcloud auth application-default login) and VERTEX_PROJECT is set.",
            fg="yellow",
        )
