...
async def main():
    ...
    await session_service.create_session(app_name="test_app", user_id="test_user", session_id="session1")

    runner = Runner(agent=agent, app_name="test_app", session_service=session_service)
    msg = Content(
        role="user",
        parts=[Part.from_text(text="Here is a long text: The agentic framework uses DDD to build memory graphs.")],
    )
    events = runner.run_async(user_id="test_user", session_id="session1", new_message=msg)
